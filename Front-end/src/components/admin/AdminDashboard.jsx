...
import { admin as adminApi, books as booksApi, authors as authorsApi, faqs as faqsApi, articles as articlesApi } from '../../services/api';
...
export default function AdminDashboard() {
  ...
  const handleDeleteAuthor = async (id) => {
    ...
    if (ok) { showToast(data.message || 'Author deleted'); loadSection('authors'); }
    else showToast('Failed to delete', 'error');
  };

  const handleSaveAuthor = async (formData) => {
    setLoading(true);
    const res = authorForm.mode === 'edit'
  ...
  const handleDeleteFaq = async (id) => {
    ...
    if (ok) { showToast(data.message || 'FAQ deleted'); loadSection('faqs'); }
    else showToast('Failed to delete', 'error');
  };

  const handleSaveFaq = async (formData) => {
    setLoading(true);
    const res = faqForm.mode === 'edit'
  ...
  const filteredAuthors = useMemo(() => {
    return authors.filter(a => a.name?.toLowerCase().includes(globalSearch.toLowerCase()) || a.role?.toLowerCase().includes(globalSearch.toLowerCase()));
  }, [authors, globalSearch]);

  const filteredFaqs = useMemo(() => {
    return faqs.filter(f => f.question?.toLowerCase().includes(globalSearch.toLowerCase()) || f.category?.toLowerCase().includes(globalSearch.toLowerCase()));
  }, [faqs, globalSearch]);

  const filteredArticles = useMemo(() => {
    return articles.filter(a => a.title?.toLowerCase().includes(globalSearch.toLowerCase()) || a.tag?.toLowerCase().includes(globalSearch.toLowerCase()) || a.excerpt?.toLowerCase().includes(globalSearch.toLowerCase()));
    ...
    <div className="section-body animate-slide-up">
      <div className="section-header-row">
        <div>
          ...
          <p className="section-subtitle">{authors.length} authors in directory</p>
        </div>
        <button className="primary-btn" onClick={() => setAuthorForm({
          mode: 'add',
          data: { name: '', role: '', bio: '', imageUrl: '', timeline: '', quote: '', masterpiece: '' }
        })}>
          <span>＋</span> Add Author
        </button>
      </div>

      {authorForm && <AuthorFormModal form={authorForm} onSave={handleSaveAuthor} onClose={() => setAuthorForm(null)} />}
      ...
      <div className="table-card">
                ...
                <td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <div style={{
                      ...
                      <img src={a.imageUrl || 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=100'} alt={a.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    </div>
                    <span style={{ fontWeight: '600', color: '#f0e6d3' }}>{a.name}</span>
                  </div>
                </td>
                <td className="text-muted">{a.role}</td>
                <td className="text-muted mono" style={{ fontSize: '0.85rem' }}>{a.timeline || '—'}</td>
                <td className="text-gold" style={{ fontSize: '0.9rem', fontWeight: '500' }}>{a.masterpiece || '—'}</td>
                <td>
                  <div className="action-btns">
                    <button className="icon-btn edit" onClick={() => setAuthorForm({ mode: 'edit', data: a })} title="Edit">✎</button>
                    <button className="icon-btn delete" onClick={() => handleDeleteAuthor(a.id)} title="Delete">⊗</button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        {filteredAuthors.length === 0 && <div className="empty-state">No authors found</div>}
      </div>
    </div>
  );
  ...
  const renderFaqs = () => (
    <div className="section-body animate-slide-up">
      <div className="section-header-row">
        ...
        <button className="primary-btn" onClick={() => setFaqForm({
          mode: 'add',
      ...
      {faqForm && <FaqFormModal form={faqForm} onSave={handleSaveFaq} onClose={() => setFaqForm(null)} />}
      ...
      <div className="table-card">
          ...
          <tbody>
            {filteredFaqs.map(f => (
              <tr key={f.id} className="table-row">
                <td style={{ fontWeight: '600', color: '#f0e6d3', whiteSpace: 'normal', verticalAlign: 'top', padding: '1rem' }}>
                  {f.question}
                </td>
                <td className="text-muted" style={{ whiteSpace: 'normal', fontSize: '0.875rem', verticalAlign: 'top', padding: '1rem', lineHeight: '1.4' }}>
                  {f.answer}
                </td>
                <td style={{ verticalAlign: 'top', padding: '1rem' }}>
                  <span className="cat-chip" style={{ fontSize: '0.75rem', letterSpacing: '0.05em' }}>{f.category}</span>
                </td>
                <td style={{ verticalAlign: 'top', padding: '1rem' }}>
                  <div className="action-btns">
                    <button className="icon-btn edit" onClick={() => setFaqForm({ mode: 'edit', data: f })} title="Edit">✎</button>
                    <button className="icon-btn delete" onClick={() => handleDeleteFaq(f.id)} title="Delete">⊗</button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        {filteredFaqs.length === 0 && <div className="empty-state">No FAQs found</div>}
      </div>
    </div>
  );
    ...
    <div className="section-body animate-slide-up">
      <div className="section-header-row">
        <div>
          ...
          <p className="section-subtitle">{articles.length} published articles</p>
        </div>
        <button className="primary-btn" onClick={() => setArticleForm({
          mode: 'add',
          data: { title: '', tag: 'CURATION', readTime: '6 min read', excerpt: '', imageUrl: '', content: '' }
        })}>
          <span>＋</span> Write Article
        </button>
      </div>

      {articleForm && <ArticleFormModal form={articleForm} onSave={handleSaveArticle} onClose={() => setArticleForm(null)} />}
      ...
      <div className="table-card">
              ...
              <tr key={a.id} className="table-row">
                <td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    ...
                    <div>
                      <p style={{ fontWeight: '600', color: '#f0e6d3', fontSize: '0.875rem' }}>{a.title}</p>
                      <p style={{ fontSize: '0.75rem', color: '#D4AF37', marginTop: '2px' }}>{a.date}</p>
                    </div>
                  </div>
                </td>
                <td className="text-muted" style={{ whiteSpace: 'normal', fontSize: '0.825rem', lineHeight: '1.4' }}>
                  {a.excerpt}
                </td>
                <td>
                  <span className="cat-chip" style={{ fontSize: '0.725rem', letterSpacing: '0.05em' }}>{a.tag}</span>
                </td>
                ...
                <td>
                  <div className="action-btns">
                    <button className="icon-btn edit" onClick={() => setArticleForm({ mode: 'edit', id: a.id, data: a })} title="Edit">✎</button>
                    <button className="icon-btn delete" onClick={() => handleDeleteArticle(a.id)} title="Delete">⊗</button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        {filteredArticles.length === 0 && <div className="empty-state">No articles found</div>}
      </div>
    </div>
  );
      ...
      <aside className="admin-sidebar">
        ...
        <nav className="admin-nav">
          <div className="nav-group">
            ...
            {SECTIONS.filter(s => ['dashboard', 'reports'].includes(s.id)).map(s => (
              <button
                key={s.id}
          ...
          <div className="nav-group">
            ...
            {SECTIONS.filter(s => ['books', 'categories'].includes(s.id)).map(s => (
              <button
                key={s.id}
                className={`nav-item ${activeSection === s.id ? 'active' : ''}`}
                onClick={() => setActiveSection(s.id)}
          ...
          <div className="nav-group">
            ...
            {SECTIONS.filter(s => ['authors', 'faqs', 'articles'].includes(s.id)).map(s => (
              <button
                key={s.id}
                className={`nav-item ${activeSection === s.id ? 'active' : ''}`}
                onClick={() => setActiveSection(s.id)}
          ...
          <div className="nav-group">
            ...
            {SECTIONS.filter(s => ['users', 'orders'].includes(s.id)).map(s => (
              <button
                key={s.id}
                className={`nav-item ${activeSection === s.id ? 'active' : ''}`}
                onClick={() => setActiveSection(s.id)}
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-card" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="modal-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem' }}>
            {fields.map(f => (
              <div key={f.key} className="modal-field" style={{ gridColumn: `span ${f.span}` }}>
                <label className="modal-label">{f.label}</label>
                {f.type === 'textarea' ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div className="modal-field">
              ...
              <input
                ...
                type="text"
                value={data.question || ''}
                onChange={e => setData(p => ({ ...p, question: e.target.value }))}
            ...
            <div className="modal-field">
              ...
              <textarea
                ...
                rows={5}
                value={data.answer || ''}
                onChange={e => setData(p => ({ ...p, answer: e.target.value }))}
...
function ArticleFormModal({ form, onSave, onClose }) {
  ...
  return (
    <div className="admin-modal-overlay">
      <div className="admin-modal-card gold-glow-border animate-scale-up" style={{ maxWidth: '650px' }}>
        <div className="modal-header">
          <h3>{form.mode === 'add' ? 'Publish Journal Article' : 'Edit Journal Article'}</h3>
          <button className="close-btn" onClick={onClose}>×</button>
        </div>
        <div className="modal-scroll-body" style={{ maxHeight: '70vh', overflowY: 'auto', padding: '1.5rem' }}>
          <div className="modal-form-grid" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <div className="modal-field">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <div className="modal-field">
                ...
                <select
                  ...
                  onChange={e => setData(p => ({ ...p, tag: e.target.value }))}
                  style={{ width: '100%' }}
                >
                  <option value="CURATION">CURATION</option>
              ...
              <div className="modal-field">
                ...
                <input
                  ...
                  onChange={e => setData(p => ({ ...p, readTime: e.target.value }))}
            ...
            <div className="modal-field">
              ...
              <input
                ...
                type="text"
                value={data.excerpt || ''}
                onChange={e => setData(p => ({ ...p, excerpt: e.target.value }))}
        ...
        <div className="modal-footer">
          <button className="modal-cancel-btn" onClick={onClose}>Cancel</button>
          <button className="modal-save-btn" onClick={() => onSave({ ...form, data })}>
